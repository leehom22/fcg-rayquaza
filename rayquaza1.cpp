...
// Manual control for energy ball //ajim
...
void drawHorizontalStripe(float radiusX, float radiusY, float zPos, float stripeWidth) { //oval stripe 
    int segments = 100;
...
void drawStraightStripe(float radius, float length, float stripeWidth, float angleDeg) { //horizontal straight line stripe 
...
void drawEnergyBall(float radius) { //darshini
    ...
    glPushMatrix();
    glColor4f(0.2f, 0.8f, 1.0f, 0.5f); // medium opacity glow
    ...
    glColor4f(0.5f, 1.0f, 1.0f, 0.9f); // brighter glow
...
void drawForearm(float length, float radius) { //darshini
    ...
    //ajim
    // Attach energy ball at tip of claw (only for right arm)
    if (!shooting && !exploding) {
        glPushMatrix();
        glTranslatef(0.0f, 0.0f, 0.2f);  // distance in front of claw tip
...
void drawRayquaza() {
    ...
    for (int i = 0; i < NUM_SEGMENTS; i++) {
        ...
        if (i == 0) {
            ...
            //added
            // draw energy ball at the tip of right hand - ajim
            if (!shooting && !exploding) {
                glPushMatrix();
                glTranslatef(0.0f, 0.0f, 0.4f); // forward from the claw
                drawEnergyBall(ballScale);
                glPopMatrix();
            };
...
void playMovingSound() {
    if (!isMoving) {
        PlaySound(TEXT("movementsound1.wav"), NULL, SND_FILENAME | SND_ASYNC | SND_LOOP);
        isMoving = true;
    }
}

void stopMovingSound() {
    PlaySound(NULL, 0, 0); // to top the looped sound
    isMoving = false;
}
...
void keyboard(unsigned char key, int x, int y) //control body move
{
    ...
    case 'd': moveDown = true; playMovingSound(); break;
    case 's': moveLeft = true;  playMovingSound(); break;
...
void update(int value)
{
    ...
    bool isAnyMovement = 
        ...
        forwardFront || forwardBack || forwardUp || forwardDown ||
...
void display(void) { //equivalant to renderScene
    ...
    glBegin(GL_QUADS);
    //glColor3d(0.13, 0.55, 0.13); // Grass Green
    glColor3d(1, 1, 1);
...
void init() {
    ...
    Image* groundImage = loadBMP("grass.bmp");
    groundTexture = loadTexture(groundImage);
    delete groundImage;
...
int main(int argc, char** argv) {
    ...
    glutCreateWindow("Rayquaza Model");
